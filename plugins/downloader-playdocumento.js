...
const fake = { quoted: {
key : {
remoteJid: '5212411347465-5212411719888@g.us',
...
if (command == 'playdocumento') {
for (let i of search.all) {
listSerch.push({title: i.title, description: `Autor: ${i.author.name} / ${i.timestamp}`, rowId: `${usedPrefix}ytmp3.2 ${i.url}`})} 
await conn.sendMessage(m.chat, listMessage, fake)}
if (command == 'playlist2') {
for (let i of search.all) {
listSerch.push({title: i.title, description: `Autor: ${i.author.name} / ${i.timestamp}`, rowId: `${usedPrefix}ytmp4.2 ${i.url}`})} 
await conn.sendMessage(m.chat, listMessage, fake)} 
} catch (e) {
