...
conn.sendButton(m.chat, `_${command}_`.trim(), author, haha.data, [['◀️️𝚂𝙸𝙶𝚄𝙸𝙴𝙽𝚃𝙴▶️', `/${command}`]], m)}
if (command == 'akiyama') {
let haha = await conn.getFile(`https://api-reysekha.herokuapp.com/api/wallpaper/akiyama?apikey=APIKEY`)
conn.sendButton(m.chat, `_${command}_`.trim(), author, haha.data, [['◀️𝚂𝙸𝙶𝚄𝙸𝙴𝙽𝚃𝙴▶️', `/${command}`]], m)}
if (command == 'anna') {
