...
try {
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: texto, contextInfo: { externalAdReply: { title: '𝚃𝙷𝙴 𝙷𝙰𝙳𝙴𝚂 𝙱𝙾𝚃', body: null, thumbnail: imagen1, sourceUrl: 'https://github.com/Yovanihades1212/HADES-BOT-MDV2.gi' }, mentionedJid: [m.sender] }}}, { quoted: m })
...
conn.sendMessage(m.chat, { video: { url: data[data.length - 1] }, caption: '𝚃𝙷𝙴 𝙷𝙰𝙳𝙴𝚂 𝙱𝙾𝚃 ', footer: await shortUrl(data[data.length - 1]), buttons }, { quoted: m })
} catch {
try {
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: texto, contextInfo: { externalAdReply: { title: '𝚃𝙷𝙴 𝙷𝙰𝙳𝙴𝚂 𝙱𝙾𝚃', body: null, thumbnail: imagen1, sourceUrl: 'https://github.com/Yovanihades1212/HADES-BOT-MDV2.git' }, mentionedJid: [m.sender] }}}, { quoted: m })
const { author: { nickname }, video, description } = await tiktokdl(args[0]).catch(async _ => await tiktokdlv2(args[0]))
const url = video.no_watermark_raw || video.no_watermark || video.no_watermark_hd || video.with_watermark
await conn.relayMessage(m.chat, prep.message, { messageId: prep.key.id, mentions: [m.sender] })
let buttons = [{ buttonText: { displayText: '𝙰𝚄𝙳𝙸𝙾' }, buttonId: `${usedPrefix}mp3` }]
conn.sendMessage(m.chat, { video: { url: url}, caption: '𝚃𝙷𝙴 𝙷𝙰𝙳𝙴𝚂 𝙱𝙾𝚃', footer: await shortUrl(url), buttons }, { quoted: m })
} catch {
